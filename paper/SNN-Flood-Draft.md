...
dB values clipped to [−50, 0] and normalized to [0, 1]; NaN → ignore-index. Chips are split into train/validation/test **stratified by region** with a fixed seed for reproducibility, using a **60/20/20 ratio → 271 / 87 / 87 chips** (agreed 09/07 — larger test set than the earlier 70/20/10 for more reliable evaluation; 1 NaN-only chip dropped → 445 valid). Spatially-overlapping chips are kept in the same split to avoid leakage. `[⚠️ Số Table 1/figures hiện là v1 (70/20/10); đang retrain 60/20/20 để cập nhật.]`
